...
//
// OTP authentication for the `clients` collection — delivered via WhatsApp.
//
// Flow:
//   1. Client POSTs { phone } to /api/custom/otp/request — server generates a
//      6-digit code, hashes it, stores it in `otp_codes` with a 5-min TTL,
//      and dispatches the code to the user's WhatsApp via green-api
//      (see pb_hooks/_lib/sms.js). Env: GREEN_API_INSTANCE, GREEN_API_TOKEN.
//   2. Client POSTs { phone, code, name?, referredBy? } to /api/custom/otp/verify.
//      Server verifies the hash, finds-or-creates the `clients` record, and
//      returns a PocketBase auth token.
//
...
routerAdd('POST', '/api/custom/otp/request', (c) => {
  ...
  // OTP delivery moved from SMS → WhatsApp (green-api). The _lib/sms.js
  // wrapper now formats the WhatsApp message itself, so we pass just the
  // 6-digit code and let it build the brand-stamped Russian template.
  try {
    sms.send(phone, code);
  } catch (err) {
    $app.logger().error('WhatsApp OTP send failed', 'phone', phone, 'err', err.message);
