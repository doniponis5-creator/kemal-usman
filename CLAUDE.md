...
### 1. POCKETBASE SCHEMA SAFETY
- NEVER delete or rename existing PocketBase collection fields — clients may have live data in them
- Only ADD new fields; never remove old ones
- If a field must be renamed, keep the old field AND add the new one with a migration comment
- NEVER change field types on existing collections without confirming data compatibility
- All collection changes must be reflected in `pb-setup.js`

### 2. API CONTRACT — NO BREAKING CHANGES
- NEVER rename existing `api.*` method names that components already call
- NEVER change the shape of objects returned by `api.*` methods
- Only ADD new methods; never remove or restructure existing ones
...
### 4. STATE & LOCALSTORAGE SAFETY
- Cart is persisted in `localStorage` key `parfum_cart` — every cart mutation MUST sync to localStorage
- `settings` object is the single source of truth for: adminPassword, deliveryCost, bonusPercent, whatsappPhone
- NEVER reset or clear `localStorage` without explicit user action

### 5. DEPLOY CHECKLIST
Before every `npm run build && scp`:
- [ ] Does `npm run build` complete with 0 errors?
- [ ] Did I change any `api.*` method? → All callers updated?
- [ ] Did I change PocketBase collection fields? → `pb-setup.js` updated?
